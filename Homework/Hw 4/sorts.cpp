...
//  Hw4
//
//  Created by Hayley Martinez on 5/25/18.
...
void insertion_sort(vector<Student>& s, bool comp(const Student&, const Student&))
{
    for ( int unsorted = 1; unsorted < s.size(); unsorted++)
    {
        ...
        s[loc] = nextItem; // Insert nextItem into sorted region
    } // end for
}
...
int main()
{
    ...
    // destruction time.
    {
        ...
        {
            pointStudents.push_back(&auxStudents[i]);
        }
        
        // sort
        sort(pointStudents.begin(), pointStudents.end(), compareStudentPtr);
        
        // clear & repopulate
        students.clear();
        for (int i = 0; i < students.size(); i++)
        {
            students.push_back(*(pointStudents[i]));
        }
